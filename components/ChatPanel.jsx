...
/**
 * ChatPanel component — handles user conversation with the Andaman AI travel assistant.
 * Sends messages to the /api/chat endpoint, renders the response itinerary,
 * and exposes the itinerary to the parent via onItineraryUpdate.
 *
 * @param {Object} props
 * @param {(itinerary: Object) => void} props.onItineraryUpdate - Callback with the latest itinerary
 */
const ChatPanel = ({ onItineraryUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const assistantMessage = {
        role: 'assistant',
        content: "Here is your suggested itinerary.",
  ...
  return (
    <section aria-label="Chat and itinerary generator" className="flex flex-col h-full basis-0 flex-grow rounded-xl border border-gray-200 overflow-hidden bg-white shadow-sm">
      {/* role="log" causes screen readers to announce new messages politely */}
      <div
        role="log"
      ...
      >
        ...
        {messages.map((m) => (
          <div
            key={m.id}
            className={`p-3 rounded-xl max-w-[85%] ${m.role === 'user' ? 'bg-blue-600 text-white self-end' : 'bg-gray-100 text-gray-800 self-start'}`}
          >
        ...
        {loading && (
          <div
            className="p-3 rounded-xl bg-gray-100 text-gray-500 self-start text-sm italic max-w-[85%]"
            aria-busy="true"
            aria-label="Generating itinerary, please wait"
          >
            Thinking...
          </div>
        )}
          ...
          <div className="w-full flex-col flex gap-2">
            <div className="p-4 rounded-xl bg-green-50 text-green-900 border border-green-200 text-sm self-start w-full">
              ...
              <details className="mt-2">
                <summary className="cursor-pointer font-medium mb-1 line-clamp-1" aria-label="View Budget Details">
                  View Budget Details
                </summary>
        ...
        {/* Scroll anchor — keeps view on latest message */}
        <div ref={messagesEndRef} />
      </div>

      <div className="p-4 bg-gray-50 border-t border-gray-200">
        <form onSubmit={handleSubmit} className="flex gap-2" aria-label="Itinerary prompt form">
          <input
            ...
            disabled={loading}
            aria-label="Enter your travel details"
